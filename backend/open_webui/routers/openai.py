...
from open_webui.metrics import number_completions_counter, total_cost_counter, input_tokens_counter, output_tokens_counter, thinking_tokens_counter, total_cost_by_provider_counter
            ...
            try:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with await session.request(
                        ...
                        number_completions_counter.labels(model=response["data"]["model"], user=user.id).inc()
                        total_cost_counter.labels(model=response["data"]["model"], user=user.id).inc(response["data"]["total_cost"])
                        total_cost_by_provider_counter.labels(provider=response["data"]["provider"]).inc(response["data"]["total_cost"])
                        input_tokens_counter.labels(model=response["data"]["model"], user=user.id).inc(response["data"]["native_tokens_prompt"])
                        output_tokens_counter.labels(model=response["data"]["model"], user=user.id).inc(response["data"]["native_tokens_completion"])
                        thinking_tokens_counter.labels(model=response["data"]["model"], user=user.id).inc(response["data"]["native_tokens_reasoning"])
                break
            except Exception as e:
                log.exception(e)
