...
total_cost_counter = Counter(
    "open_webui_cost_total",
    "Total cost of chat completions",
    ["model", "user"]
)
...
input_tokens_counter = Counter(
    "open_webui_input_tokens_total",
    "Total number of input tokens",
    ["model", "user"]
)
output_tokens_counter = Counter(
    "open_webui_output_tokens_total",
...
number_pending_users_total.set_function(lambda: Users.count_users(filter_role="pending"))
...
def create_prometheus_server(port):
    ...
    httpd = exposition.make_server("", port, app, exposition.ThreadingWSGIServer, handler_class=exposition._SilentHandler)
